...
uint64_t getRealOffset(uint64_t offset){
    // TARGET UNITY FRAMEWORK AUTOMATICALLY
    // Most offsets provided (Bypass/Il2Cpp) are for UnityFramework
    uint64_t address = getAbsoluteAddress("UnityFramework", offset);
    if (address == 0) {
        // Fallback to main binary if UnityFramework isn't found
        address = getAbsoluteAddress(NULL, offset);
    }
    return address;
}
