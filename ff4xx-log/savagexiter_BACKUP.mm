...
- (void)setSpeedMode:(int)mode {
    ...
    if (!g_MemoryEngineReady) {
        NSLog(@"[ST-SPEED] ⚠️ Memory engine not ready, skipping");
        return;
    }
    
    if (g_FeatureFailCount >= MAX_FAIL_COUNT) {
        NSLog(@"[ST-SPEED] ⚠️ Too many failures, feature disabled");
        return;
    }
...
- (void)toggleFastScope:(BOOL)enable {
    NSLog(@"[ST-FASTSCOPE] ⚠️ DISABLED for stability - crashes detected");
    return;
...
- (void)toggleTracking:(BOOL)enable {
    NSLog(@"[ST-TRACKING] ⚠️ DISABLED for stability - crashes detected");
    return;
...
- (void)toggleNoRecoil:(BOOL)enable {
    NSLog(@"[ST-NORECOIL] ⚠️ DISABLED for stability - crashes detected");
    return;
